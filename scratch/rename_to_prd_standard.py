...
# Current name -> New content-based standard name
mapping = {
    "PRD AI Chatbot for e-commerce.md": "prd-ai-chatbot.md",
    "PRD AI Chatbot for e-commerce (1).md": "prd-ai-chatbot-intro.md",
    "PRD AI Chatbot for e-commerce (2).md": "prd-reminder-agent.md",
    "PRD AI Chatbot for e-commerce (3).md": "prd-image-upload-feasibility.md",
    "PRD AI Chatbot for e-commerce (4).md": "prd-file-processing.md",
    "PRD AI Chatbot for e-commerce 9.md": "prd-tool-calling-architecture.md"
}
...
def get_replacements(old, new):
    ...
    replaces.append((quoted_old, new))
    
    # 3. Custom quote variations in markdown URLs
    quoted_old_c1 = old.replace(" ", "%20").replace("[", "%5B").replace("]", "%5D").replace("(", "%28").replace(")", "%29")
    if (quoted_old_c1, new) not in replaces:
        replaces.append((quoted_old_c1, new))
        
    quoted_old_c2 = old.replace(" ", "%20").replace("[", "%5B").replace("]", "%5D")
    if (quoted_old_c2, new) not in replaces:
        replaces.append((quoted_old_c2, new))
